...
type Props = {
  visible:    boolean;
  onClose:    () => void;
  onSubmit:   (data: { type: OfferType; subject: string; description: string }) => Promise<void>;
  editTarget: EditTarget;
};

export default function AddOfferModal({ visible, onClose, onSubmit, editTarget }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (!description.trim()) { setError('Opisz temat / dział'); return; }
    setError(null);
    setLoading(true);
    try {
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={handleClose}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={ss.kavWrap}>
        <View style={[ss.sheet, { backgroundColor: C.surface, borderColor: C.border }]}>

          <View style={[ss.handle, { backgroundColor: C.border }]} />

          <View style={ss.header}>
            <Text style={[ss.title, { color: C.text1 }]}>
              {isEdit ? 'Edytuj ofer' : 'Nowa ofer'}
              <Text style={{ color: C.cyan }}>tę</Text>
            </Text>
            <TouchableOpacity onPress={handleClose} style={[ss.closeBtn, { backgroundColor: C.surface2, borderColor: C.border }]}>
              <Text style={[ss.closeTxt, { color: C.text2 }]}>✕</Text>
            </TouchableOpacity>
          </View>

          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={ss.body}>

            {/* TYPE TOGGLE */}
            <Text style={[ss.label, { color: C.text3 }]}>// typ oferty</Text>
            <View style={[ss.toggle, { backgroundColor: C.surface2, borderColor: C.border }]}>
              {(['teach', 'learn'] as OfferType[]).map(t => {
                ...
                return (
                  <TouchableOpacity
                    key={t}
                    style={[ss.toggleBtn, active && { backgroundColor: activeBg, borderWidth: 1, borderColor: activeBdr }]}
                    onPress={() => setType(t)}
                    activeOpacity={0.75}
                  >
                    <Text style={ss.toggleEmoji}>{t === 'teach' ? '🎓' : '📚'}</Text>
                    <Text style={[ss.toggleTxt, { color: active ? activeCol : C.text3 }]}>
                      {t === 'teach' ? 'Uczę kogoś' : 'Chcę się nauczyć'}
                    </Text>
                    <Text style={[ss.toggleSub, { color: active ? activeCol : C.text3, opacity: 0.7 }]}>
            ...
            <View style={ss.subjectGrid}>
              {SUBJECTS.map(s => {
                ...
                return (
                  <TouchableOpacity
                    key={s.key}
                    style={[ss.subjectBtn, { backgroundColor: C.surface2, borderColor: C.border },
                      active && { backgroundColor: activeBg, borderColor: activeBdr }]}
                    onPress={() => setSubject(s.key)}
                    activeOpacity={0.75}
                  >
                    <Text style={ss.subjectEmoji}>{s.emoji}</Text>
                    <Text style={[ss.subjectTxt, { color: active ? activeCol : C.text2 }]}>{s.label}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
            ...
            <TextInput
              style={[ss.input, { backgroundColor: C.surface2, borderColor: C.border, color: C.text1 }]}
              value={description}
            ...
            {error && (
              <View style={[ss.errorBox, { backgroundColor: C.roseBg, borderColor: C.roseBdr }]}>
                <Text style={[ss.errorTxt, { color: C.rose }]}>{error}</Text>
              </View>
            )}

            <TouchableOpacity
              style={[ss.submitBtn, { backgroundColor: type === 'teach' ? C.cyan : C.lime }, loading && { opacity: 0.6 }]}
            ...
            >
              {loading ? (
                ...
                <Text style={[ss.submitTxt, { color: C.bg }]}>
                  {isEdit ? 'Zapisz zmiany →' : type === 'teach' ? 'Dodaj ofertę nauki →' : 'Szukaj nauczyciela →'}
                </Text>
              )}
            </TouchableOpacity>
...
const ss = StyleSheet.create({
  ...
  label:    { fontSize: 9, fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginTop: 8 },

  toggle:     { flexDirection: 'row', borderRadius: 12, borderWidth: 1, padding: 4, gap: 6 },
  toggleBtn:  { flex: 1, borderRadius: 9, paddingVertical: 10, alignItems: 'center', gap: 3 },
  ...
  subjectBtn:   { borderWidth: 1, borderRadius: 10, paddingHorizontal: 10, paddingVertical: 7, flexDirection: 'row', alignItems: 'center', gap: 5 },
  subjectEmoji: { fontSize: 14 },
  subjectTxt:   { fontSize: 10, fontFamily: 'monospace', fontWeight: '500' },

  input:    { borderWidth: 1, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, fontSize: 13, fontFamily: 'monospace', minHeight: 100 },
  errorBox: { borderWidth: 1, borderRadius: 8, padding: 10 },
  errorTxt: { fontSize: 11, fontFamily: 'monospace' },
  submitBtn:{ borderRadius: 13, paddingVertical: 14, alignItems: 'center', marginTop: 8 },
